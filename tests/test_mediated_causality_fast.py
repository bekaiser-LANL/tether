...
from tether.benchmarks.mediated_causality import (
    causality_from_frequency,
    causality_from_table,
    generate_dataset_by_difficulty,
)
...
def test_causality_aginst_equation_input():
    # Random positive numbers
    array = np.array([285.0, 97.0, 216.0, 200.0, 211.0, 57.0, 94.0, 97.0])
...
def test_generate_easy():
    ...
    assert np.all(
        (np.isnan(result)) | (result >= 1)
    ), "All values should be >= 1 or NaN"
...
def test_generate_medium():
    difficulty_threshold = np.array([0.05, 0.25])
    factor_range = np.linspace(10, 20, 5)
    result = generate_dataset_by_difficulty(
        "medium", difficulty_threshold, factor_range
    )
    assert result.shape == (8,), "Result should have 8 elements"
    assert np.all(
        (np.isnan(result)) | (result >= 1)
